         ...
         function GetAlert()
{
   ...
   $method = "GET";
   $url = "https://oncall-prod-us-central-0.grafana.net/oncall/api/v1/alerts";
   $curl = curl_init( $url);
   ...
   curl_setopt($curl, CURLOPT_HTTPHEADER, array(
         'Content-Type: application/json',
         'Authorization: b6604d308cdce598b378d5be2d4ed237386c1b777b2a397941571a1d4b4d0980'
   ));
   ...
   if(!$response)
   {
      error_log("Get Alert Connection Failure", 3, "./Log/" . date("Ymd") . ".log");
    die("Get Alert Connection Failure");
   }  
   
 
   if($http_status != 200)
   {
    error_log("\n". date("d/m/Y") . " ". date("h:i:s") . " : " . " Get Alert Error Reason : " .  $array2['message'], 3, "./Log/" . date("Ymd") . ".log");
   }
   curl_close($curl);
   return $response;
